...
public class Events implements Listener {
	...
	public void onCreatureSpawnEvent(CreatureSpawnEvent event){
		...
		if (event.getSpawnReason() == CreatureSpawnEvent.SpawnReason.SPAWNER){
			...
			//plugin.info("onCreatureSpawnEvent chunkEntities.size(): " + chunkEntities.size());
			if (chunkEntities.size() > plugin.config.maxmobperchunk){
				...
				for (int i = chunkEntities.size() - 1; i >= plugin.config.maxmobperchunk; i--) {
					...
					//plugin.info("onCreatureSpawnEvent Removing mob that lived: " + (entity.getTicksLived()/20) + " seconds.");
					entity.remove();
				}
			}
		}
